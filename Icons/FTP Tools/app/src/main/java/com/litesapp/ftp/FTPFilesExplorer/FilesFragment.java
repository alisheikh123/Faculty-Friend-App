...
public abstract class FilesFragment<E> extends Fragment implements ActionMode.Callback, SearchView.OnQueryTextListener{
    ...
    public void selectItem(View v){
        ...
        actionMode.getMenu().findItem(R.id.action_rename_file).setVisible(false);

        //TODO: here i changing the visibility of rename menu item from (oneItem) => to false

        if(oneItem) {
            oneItem &= (((MainActivityClient) getActivity()).isRemoteAlive) ||(((MainActivityClient) getActivity()).isLocalAlive && ((File) (filesAdapter.getSelectedItems().get(0))).isFile());
    ...
    public void onCreateOptionsMenu(Menu menu, MenuInflater inflater) {
        ...
        ((SearchView) MenuItemCompat.getActionView(item)).setOnSearchClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                ((MainActivityClient)getActivity()).fab.hide();
            }
        });
        ((SearchView) MenuItemCompat.getActionView(item)).setOnCloseListener(new SearchView.OnCloseListener() {
            ...
            public boolean onClose() {
                ...
                ((MainActivityClient)getActivity()).fab.show();

                //TODO: above I change thre show() to hide()
                return false;
            }
        });
