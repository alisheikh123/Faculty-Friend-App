...
public class EditConnectionFragment extends Fragment {
        ...
        switch (item.getItemId()) {
            case R.id.action_done_edit_connection:
                if (validateInput()) {
                    ...
                    ((MainActivityClient) getActivity()).finishEditConnection(toEdit, edited);
                } else {
    ...
    public void validatePort() {
        ...
        if (til.getEditText().getText().toString().isEmpty()) {
            ...
            til.requestFocus();
        } else if(til.getEditText().getText().toString().length()>5){
                til.setError(getString(R.string.port_length_error));
        } else {
